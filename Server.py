...
with seats_tab:
    ...
    with st.form("add_seat_form"):
        new_seat = st.text_input("New Seat ID (e.g., A1, B2)")
    ...
    if seats:
        with st.container(border=40):
            for seat in seats:
                ...
                col1.write(f"<div><b><h5>》 Seat: {seat['seat_id']}</h5></b></div>", unsafe_allow_html=True)
...
with orders_tab:
    ...
    if not orders:
        ...
        for order in orders:
            with st.container(border=10):
                ...
                st.markdown(f"<b>📋 Item: {format_orders(order['items'])}</b>", unsafe_allow_html=True)
                st.markdown(f"⏱️ Time: {order['timestamp']}")
    ...
    if st.button("❌ Delete All Orders"):
        dummy_uuid = "00000000-0000-0000-0000-000000000000"  # valid UUID format
